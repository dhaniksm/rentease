...
class _UserPaymentScreenState extends State<UserPaymentScreen> {
  ...
  static const String prodUrl = 'https://rentase-api.vercel.app';
  ...
  // Helper to test connectivity and get appropriate base URL
  Future<String> _getWorkingBaseUrl() async {
    // Try Vercel production URL first, if not we fallback to local emulators
    try {
      final response = await http.get(Uri.parse('$prodUrl/api/payments')).timeout(const Duration(seconds: 2));
      if (response.statusCode == 200) return prodUrl;
    } catch (_) {}

    try {
      final response = await http.get(Uri.parse('$localUrl/api/payments')).timeout(const Duration(seconds: 1));
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true && jsonResponse['data'] != null) {
          ...
          final allRentals = dataList.map((json) => UserRentalModel.fromJson(json)).toList();
          
          setState(() {
            _myRentals = allRentals.where((r) => r.userId == currentUserId).toList();
  ...
  Future<void> _confirmPayment(String rentalId) async {
    ...
    try {
      ...
      final response = await http.patch(
        Uri.parse('$baseUrl/api/payments/confirm/$rentalId'),
      );

      if (response.statusCode == 200) {
        _showSnackBar('Konfirmasi pembayaran terkirim! Menunggu verifikasi admin.', Colors.green);
        _fetchMyRentals();
      } else {
        final decoded = json.decode(response.body);
        _showSnackBar(decoded['message'] ?? 'Gagal melakukan konfirmasi.', AppColors.maroon);
        _fetchMyRentals();
      }
    } catch (e) {
                ...
                child: _isLoading && _myRentals.isEmpty
                    ...
                    : _myRentals.isEmpty
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                              color: const Color(0xFF6B0014), // Premium dark maroon/red container
            ...
            if (_myRentals.isNotEmpty)
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ElevatedButton(
                      onPressed: _isLoading || _myRentals[_selectedCardIndex].rentalStatus != 'unpaid'
                          ? null
                          : () {
                              _confirmPayment(_myRentals[_selectedCardIndex].id);
                            },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: const Color(0xFF6B0014),
