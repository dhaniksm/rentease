...
class _RiwayatUserScreenState extends State<RiwayatUserScreen> {
  ...
  Future<void> fetchHistoryData() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        historyData = rentalProvider.rawRentals.where((rental) {
          final status = rental['status'] ?? rental['rental_status'];
          return status == 'active' || status == 'returned' || status == 'cancelled';
        }).toList();
      });
    } catch (e) {
      if (!mounted) return;
              ...
              child: isLoading && historyData.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final status =
                            rental['status'] ?? rental['rental_status'];
                                            ...
                                            child: ElevatedButton(
                                              onPressed: () {
                                                Navigator.push(
                                                  context,
                                                  MaterialPageRoute(
                                                    builder: (_) => const UserPaymentScreen(),
                                                  ),
                                                ).then((_) {
                                                  if (mounted) fetchHistoryData();
                                                });
                                              },
